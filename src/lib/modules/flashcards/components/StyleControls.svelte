...
<div class="style-controls">
  ...
  <details open>
    <summary>Spacing &amp; layout</summary>
    <div class="rows">
  ...
  {@render fontGroup('Title font', s.titleFont, (p) => setSettings({ titleFont: { ...s.titleFont, ...p } }))}
  {@render fontGroup('Content font', s.contentFont, (p) => setSettings({ contentFont: { ...s.contentFont, ...p } }))}
...
{#snippet fontGroup(title: string, f: FontSpec, patch: (p: Partial<FontSpec>) => void)}
  <details>
...
<style>
  .style-controls { display:flex; flex-direction:column; gap:8px; padding:12px;
    background:var(--surface); border-bottom:1px solid var(--border); }
  details { border:1px solid var(--border); border-radius:8px; background:var(--bg); }
  summary { list-style:none; cursor:pointer; padding:7px 10px; font-size:11px; font-weight:600;
    letter-spacing:.04em; text-transform:uppercase; color:var(--text-muted); user-select:none; }
  summary::-webkit-details-marker { display:none; }
  summary::before { content:'▸'; display:inline-block; margin-right:6px; transition:transform .12s ease; color:var(--text-muted); }
  details[open] summary::before { transform:rotate(90deg); }
  summary:hover { color:var(--accent); }
  summary:focus-visible { outline:2px solid var(--accent); outline-offset:-2px; border-radius:8px; }
  .rows { display:flex; flex-direction:column; gap:6px; padding:2px 10px 10px; }
  ...
  .row select { max-width:130px; justify-self:end; }
